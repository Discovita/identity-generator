...
class TestImageGenerationService:
    ...
    def setup_method(self):
        ...
        self.service = ImageGenerationService(open_ai_service=self.mock_client)
    ...
    def test_safe_generate_scene_success(self):
        ...
        assert result.data.data[0].url == "https://example.com/safe-image.jpg"
        assert result.data.data[0].revised_prompt == "A safe image as requested."
        assert result.original_prompt is not None

    def test_safe_generate_scene_empty_result(self):
        """Test safe_generate_scene with empty result - should fail fast."""
        # Mock empty response
        self.mock_client.generate_image.return_value = []

        # Call safe_generate_scene - expect AssertionError due to fail-fast
        with pytest.raises(AssertionError, match="No image was generated by OpenAI"):
            self.service.safe_generate_scene(
                setting="gym", outfit="workout clothes", emotion="energetic"
            )

    def test_safe_generate_scene_error_handling(self):
        """Test error handling in safe_generate_scene - should propagate exception."""
        # Mock an exception
        self.mock_client.generate_image.side_effect = Exception("API error")

        # Call safe_generate_scene - expect exception to propagate
        # The pytest.raises context manager IS the assertion - it verifies the exception is raised
