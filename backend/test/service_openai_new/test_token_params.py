...
class TestTokenParams:
    """Tests for handling of token parameters in OpenAIClient."""
    ...
    def helper(self, mock_client):
        """Create an OpenAIClient instance with a mocked client."""
        with patch("discovita.service.openai_new.core.base.OpenAI"):
            helper = OpenAIClient(api_key="fake_key", organization="fake_org")
    ...
    def test_standard_model_uses_max_tokens(self, helper):
        ...
        helper.create_chat_completion(
            prompt="Hello", model="gpt-4-turbo", max_tokens=100
        )
    ...
    def test_o_model_uses_max_completion_tokens(self, helper):
        ...
        helper.create_chat_completion(prompt="Hello", model="o3-mini", max_tokens=100)
    ...
    def test_explicitly_provided_max_completion_tokens(self, helper):
        ...
        helper.create_chat_completion(
            prompt="Hello", model="o3-mini", max_tokens=100, max_completion_tokens=200
        )
    ...
    def test_error_recovery(self, helper):
        ...
        # Instead of checking the arguments in the call, let's check what happens to the API response
        helper.client.chat.completions.create.side_effect = [
            ValueError(
        ...
        # This should trigger the error but recover automatically
        response = helper.create_chat_completion(
            prompt="Hello", model="o3-mini", max_tokens=100
        )

        # Check that we got a response after recovery
        assert response == "test response after recovery"

        # Check that we were called twice - once with the error, once successfully
        assert helper.client.chat.completions.create.call_count == 2
    ...
    def test_custom_model_detection(self, helper):
        ...
        helper.create_chat_completion(
            prompt="Hello", model="o3-something-custom", max_tokens=100
        )
        ...
        helper.create_chat_completion(
            prompt="Hello", model="gpt-4o-custom", max_tokens=100
        )
