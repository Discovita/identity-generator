...
def check_dependency_versions():
    ...
    mute_warning = os.getenv("MUTE_OPENAI_SERVICE_WARNING", "False").lower() in (
        "true",
        "1",
        "t",
    )
    ...
    if not mute_warning and not is_compatible:
        log.warning(
            f"The 'OpenAIClient' tool was created using openai version {OPENAI_VERSION}. "
        ...
        log.info(
            "This warning can be muted by setting the MUTE_OPENAI_SERVICE_WARNING "
