...
from discovita.service.openai.client.responses_client import get_response_with_responses
...
class OpenAIClient:
    ...
    def __init__(
        ...
        self.client = AsyncOpenAI(
            api_key=api_key, base_url=base_url, timeout=60.0, max_retries=3
        )
    ...
    async def get_structured_completion(
        ...
        return await self.ops.get_structured_completion(
            self.client, messages, response_model
        )
    ...
    async def describe_image_with_vision(
        self, image_url: AnyHttpUrl, prompt: str
    ) -> str:
    ...
    get_response_with_responses = get_response_with_responses

    # Legacy method aliases for backward compatibility
    get_structured_response_with_responses = get_response_with_responses
    # Legacy method aliases for backward compatibility
    get_structured_response_with_responses = get_response_with_responses
